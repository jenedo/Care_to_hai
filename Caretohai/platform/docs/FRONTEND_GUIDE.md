...
- **API client:** `@asaancare/api-client-react` (generated from OpenAPI)
...
```bash
cd platform
PORT=5000 BASE_PATH=/ pnpm --filter @asaancare/admin-dashboard run dev
```
...
Uses **generated TanStack Query hooks** from `@asaancare/api-client-react`.
...
```bash
pnpm --filter @asaancare/api-spec run generate
```
...
**File:** `artifacts/admin-dashboard/vite.config.ts`
...
4. Use generated hooks from `@asaancare/api-client-react` for data
