...
router.patch(
  ...
  async (req, res): Promise<void> => {
    ...
    try {
      ...
      if (session.doctorId !== auth.doctorId) {
        res.status(403).json({ error: "Forbidden" });
        return;
      }
...
/* ─── Patient: pay for extra time (disabled until payment gateway) ─── */
router.patch(
  ...
  async (req, res): Promise<void> => {
    ...
    try {
      ...
      if (session.patientId !== auth.patientId) {
        res.status(403).json({ error: "Forbidden" });
        return;
      }

      res.status(501).json({
        error: "Payment gateway not integrated. Use subscription credits or contact support.",
