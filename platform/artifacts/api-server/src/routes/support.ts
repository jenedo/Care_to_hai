...
router.get("/support/stats", async (_req, res) => {
  ...
  const closedWeek = all.filter(t => t.status === "RESOLVED" || t.status === "CLOSED").length;
  const urgentOpen = all.filter(t => t.priority === "URGENT" && t.status === "OPEN").length;
  const resolved = all.filter(t => t.status === "RESOLVED" || t.status === "CLOSED").length;
  const slaCompliance = all.length > 0 ? Math.round((resolved / all.length) * 100) : 0;

  res.json({
    avg_response_time: null,
    avg_resolution_time: null,
    sla_breaches: urgentOpen,
    tickets_closed_week: closedWeek,
