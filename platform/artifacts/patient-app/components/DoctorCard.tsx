...
export function DoctorCard({ item, onPress }: DoctorCardProps) {
  ...
  const onlineStatus = item.onlineStatus ?? (item.is_available_online ? "ONLINE" : "OFFLINE");
  const isOnline = onlineStatus === "ONLINE";
  const isBusy = onlineStatus === "BUSY";
  const dotColor = isOnline ? "#22C55E" : isBusy ? "#F59E0B" : "#94A3B8";
    ...
    >
      ...
      <View style={[styles.footer, { borderTopColor: colors.border }]}>
        <Text style={[styles.viewBtn, { color: colors.primary }]}>
          {isOnline ? "Chat Now →" : "View Profile →"}
        </Text>
        <View style={styles.metaItem}>
          <View style={[styles.statusDotSmall, { backgroundColor: dotColor }]} />
          <Text style={[styles.metaText, { color: isOnline ? "#16A34A" : isBusy ? "#D97706" : colors.mutedForeground }]}>
...
const styles = StyleSheet.create({
  ...
  statusDotSmall: { width: 7, height: 7, borderRadius: 4, marginRight: 4 },
});
