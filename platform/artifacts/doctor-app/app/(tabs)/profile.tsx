...
export default function ProfileScreen() {
  ...
  const { doctor, logout, token, refreshProfile } = useAuth();
  ...
  const uploadVerificationDocument = async (docType: string) => {
    if (!token) {
    ...
    form.append("file", {
      uri: asset.uri,
      name: `${docType}-${Date.now()}.jpg`,
      type: "image/jpeg",
    } as unknown as Blob);
    form.append("type", docType);

    try {
      const uploadRes = await fetch(`${API_BASE}/api/uploads/doctor-document`, {
        method: "POST",
      ...
      if (!uploadRes.ok || !uploadJson?.data?.url) {
        Alert.alert("Upload failed", uploadJson?.error?.message ?? "Could not upload file.");
        return;
      }

      const saveRes = await fetch(`${API_BASE}/api/doctor/verification/document`, {
        method: "POST",
        headers: {
          Authorization: `Bearer ${token}`,
          "Content-Type": "application/json",
        },
        body: JSON.stringify({ type: docType, url: uploadJson.data.url }),
      });
      if (!saveRes.ok) {
        const saveJson = await saveRes.json();
        Alert.alert("Save failed", saveJson?.error ?? "Document uploaded but not saved.");
        return;
      }
      ...
      Alert.alert(
        "Document submitted",
        "Your document was uploaded and sent for admin review.",
      );
    } catch {
  ...
  const handleDocumentRequest = () => {
    Alert.alert(
      ...
      "Select the document type to upload. Admin will review within 24 hours.",
      [
        { text: "Cancel", style: "cancel" },
        { text: "CNIC Front", onPress: () => uploadVerificationDocument("cnic_front") },
        { text: "CNIC Back", onPress: () => uploadVerificationDocument("cnic_back") },
        { text: "Degree", onPress: () => uploadVerificationDocument("degree") },
        { text: "Certificate", onPress: () => uploadVerificationDocument("certificate") },
      ],
    );
  };
