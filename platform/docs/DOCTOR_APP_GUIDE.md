...
- **Data fetching:** TanStack Query v5 (via `@asaancare/api-client-react`)
- **Language:** TypeScript
...
const TOKEN_KEY = "asaancare_doctor_token";
...
Set in `app/_layout.tsx` via `setBaseUrl` from `@asaancare/api-client-react`. In development, this points to the Replit API server URL (not localhost, since the device/emulator needs the public URL).
...
To run locally:
```bash
cd platform/artifacts/doctor-app
pnpm install
npx expo start
```
...
The doctor app already uses `@asaancare/api-client-react` generated hooks. These call the same API server as the admin frontend. The key difference is authentication method (Bearer token vs cookie).
